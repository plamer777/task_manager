...
class BoardPermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method in permissions.SAFE_METHODS:
            ...
            return Participant.objects.filter(
                user=request.user, board=obj, role=Roles.owner).exists()
...
class CategoryCreatePermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return Participant.objects.filter(
            ...
            role__in=[Roles.owner, Roles.writer]).exists()
...
class CommentCreatePermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return Participant.objects.select_related('user').filter(
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method in permissions.SAFE_METHODS:
            return Comment.objects.select_related('goal').filter(
                ...
                goal__category__board=obj.goal.category.board).exists()
        else:
            return Comment.objects.select_related('goal').filter(
                goal__category__board__participants__user=request.user,
